...
public class IngresoEgresoBean implements Serializable {
    ...
    public void iniciar() {
        ...
        modelo = new LazyDataModel<>() {
            @Override
            public int count(Map<String, FilterMeta> f) { return (int) service.contar(filtroGlobal, filtroTipo); }
            @Override
            public List<IngresoEgreso> load(int first, int size, Map<String, SortMeta> s, Map<String, FilterMeta> f) {
                return service.listar(first, size, filtroGlobal, filtroTipo);
            }
        };
    }
