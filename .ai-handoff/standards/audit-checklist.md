...
- Si el REQ es critico, se documento la fuente: ID `RN-*` de docs-migracion, archivo C# (`FrmX.cs`/`XService.cs`), SP Firebird (`RPT_*`) o valor de `DOMINIOS` (doc 07).
- Se listaron las reglas heredadas encontradas.
- Se justificaron las diferencias respecto del legado (los 7 bugs conocidos se corrigen siempre, sin justificacion extra; ver CLAUDE.md).
