...
scp -O $war sginmo-vps:apps/wildfly-40.0.0.Final/standalone/deployments/
if ($LASTEXITCODE -ne 0) { Write-Error 'scp fallo'; exit 1 }

ssh -o BatchMode=yes sginmo-vps 'for i in $(seq 1 30); do [ -f ~/apps/wildfly-40.0.0.Final/standalone/deployments/sginmo-web.war.deployed ] && break; sleep 2; done; curl -s -o /dev/null -w "Verificacion VPS: HTTP %{http_code}\n" http://localhost:8080/sginmo-web/'
